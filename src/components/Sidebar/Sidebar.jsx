...
function Sidebar() {
  ...
  return (
    <aside
      className={`w-[9%] fixed top-0 left-0 h-screen bg-black-sidebar p-5
       pt-8 `}
    >
      <div
      ...
      >
        ...
        />
        <h1
          className={` origin-left font-mono text-2xl font-medium text-white duration-300 `}
        >
          ChillM
        </h1>
      </div>
      ...
      <div className="pt-2">
        {Menus.map((Menu, index) => (
          <Link className="" to={Menu.link}>
            <button
              key={index}
              className={` flex w-full cursor-pointer items-center gap-x-4 rounded-md  p-2 text-sm text-gray-300 hover:bg-light-white focus:bg-light-white ${
                Menu.gap ? "mt-12" : "mt-2 "
              }`}
